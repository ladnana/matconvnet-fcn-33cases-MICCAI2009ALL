...
filters = single(bilinear_u(4, 1, 3)) ;%21->3 edited by mR
...
net.addLayer('skip4', ...
     dagnn.Conv('size', [1 1 512 3], 'pad', 0), ...
     'x24', 'x40', {'skip4f','skip4b'});%21->3 edited by mR

f = net.getParamIndex('skip4f') ;
net.params(f).value = zeros(1, 1, 512, 3, 'single') ;%21->3 edited by mR
net.params(f).learningRate = 0.1 ;
net.params(f).weightDecay = 1 ;

f = net.getParamIndex('skip4b') ;
net.params(f).value = zeros(1, 1, 3, 'single') ;%21->3 edited by mR
...
filters = single(bilinear_u(32, 3, 3)) ;%21->3 edited by mR
net.addLayer('deconv16', ...
  dagnn.ConvTranspose('size', size(filters), ...
             ...
             'x41', 'prediction', 'deconv16f') ;%21->3 edited by mR
