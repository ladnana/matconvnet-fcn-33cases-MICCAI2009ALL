...
filters = single(bilinear_u(4, 1, 3)) ;%not sure that do 'crop' need to be modified ?
...
net.addLayer('skip1', ...
     dagnn.Conv('size', [1 1 64 3]), ...
     'x5', 'x49', {'skip1f','skip1b'});%21->3 edited by mR

f = net.getParamIndex('skip1f') ;
net.params(f).value = zeros(1, 1, 64, 3, 'single') ;%'learningRate' need to be modified?
...
filters = single(bilinear_u(4, 3, 3)) ;%'crop'modified from 8 to 4 to 2 to 1?
