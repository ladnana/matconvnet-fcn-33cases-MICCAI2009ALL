...
net.addLayer('skip3', ...
     dagnn.Conv('size', [1 1 256 3]), ...
     'x17', 'x43', {'skip3f','skip3b'});%21->3 edited by mR

f = net.getParamIndex('skip3f') ;
net.params(f).value = zeros(1, 1, 256, 3, 'single') ;%21->3 edited by mR
...
filters = single(bilinear_u(16, 3, 3)) ;%21->3 edited by mR
net.addLayer('deconv8', ...
  dagnn.ConvTranspose('size', size(filters), ...
             ...
             'x44', 'prediction', 'deconv8f') ;%21->3 edited by mR
